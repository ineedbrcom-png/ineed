...
export const requestNotificationPermissionAndNotify = async (messageCount: number = 0) => {
    ...
    const handlePermission = (permission: NotificationPermission) => {
        if (permission === 'granted') {
            if (messageCount > 0) {
                 new Notification('iNeed - Novas Mensagens!', {
                    body: `Você tem ${messageCount} mensagens não lidas.`,
                    icon: APP_ICON_URI,
                });
            } else {
                 new Notification('iNeed - Notificações Ativadas!', {
                    body: 'Você agora receberá atualizações importantes.',
                    icon: APP_ICON_URI,
                });
            }
        }
